...
void *listen_thread(void *arg) {
	epoll_fd_t *ep_t = (epoll_fd_t*)arg;
    int epoll_fd = ep_t->epollfd;
	int i = 0;
	int bindCore = ep_t->epollid%cpu_core_num;
	printf("ep_t->epollid=%d,cpu_core_num=%d ,bindCore= %d \n",ep_t->epollid,cpu_core_num,bindCore);
	
	struct epoll_event events[MAX_EPOLLSIZE];
    bindCpuCore(pthread_self(),bindCore);
	
	 int retCore= findThreadAtWhichCpuCore(pthread_self(),cpu_core_num);
	 if (retCore>=0)
	...
	while (1) {
		...
		for (i = 0;i < nfds;i ++) {
			...
			if (sockfd) {
				...
				//两种工作模式
				1//1 在程序没有达到连接数制定目标前，为了不影响连接建立，将读写任务分解到别的线程
				// 2 在连接总数全部完成，那么转换为 同一个Epoll线程里面执行
				if (nRun) {
...
epoll_fd_t epoll_fds[CPU_CORES_SIZE] = {0};
pthread_t thread_id[CPU_CORES_SIZE] = {0};
	
	////////////////////////////////bind acceptor-epoller-to-epoll worker  start////////////////////////////////////////////// 
	int j = 0;  //防止
	for (j = 0;j < CPU_CORES_SIZE;j++) {
		epoll_fds[j].epollfd = epoll_create(MAX_EPOLLSIZE);
		epoll_fds[j].epollid = j;
        //pthread_create(&thread_id[j], NULL, listen_thread, &i); 传递i要注意，这个i被多个线程共享
		//会导致 逻辑混乱
		pthread_create(&thread_id[j], NULL, listen_thread, &epoll_fds[j]);
	}
	...
	for (i = 0;i < MAX_PORT;i ++) {
		...
		epoll_ctl(epoll_fds[i%CPU_CORES_SIZE].epollfd, EPOLL_CTL_ADD, sockfd, &ev);  
	}
